...
const getInstances = ({ allFormats, options }: { allFormats: string[]; options: Ajv.Options }) =>
  allFormats.map(schema => {
    if (typeof schema === "string") return new Ajv(options).compile({ format: schema });
    else return new Ajv(options).compile(schema);
  });
...
export const returnFormats = (values: string[]) => {
  ...
  return instances
    .map((validate, i) => {
      ...
      if (allMatch.length === 1 && allMatch[0] === true) return allFormats[i];
    })
    .filter(d => d);
};
